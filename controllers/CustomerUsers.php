...
class CustomerUsers
{
    ...
    public function register()
    {
        ...
        if (
            ...
            $_SESSION['flash-msg'] = 'fill-fields';
        ...
        if (!filter_var($data['usersEmail'], FILTER_VALIDATE_EMAIL)) {
            $_SESSION['flash-msg'] = 'email-invalid';
            redirect("../CustomerInventory.php");
            die();
        }

        if($data['usersPwd'] != $data['pwdRepeat']){
            $_SESSION['flash-msg'] = 'password-unmatched';
            redirect("../CustomerInventory.php");
            die();
        }

        if (strlen($data['usersPwd']) < 8) {
            $_SESSION['flash-msg'] = 'password-invalid';
            redirect("../CustomerInventory.php");
            die();
        }

        if (!preg_match('/[A-Z]/', $data['usersPwd'])) {
            $_SESSION['flash-msg'] = 'password-invalid';
            redirect("../CustomerInventory.php");
            die();
        }
        ...
        if (!preg_match('/[0-9]/', $data['usersPwd'])) {
            $_SESSION['flash-msg'] = 'password-invalid';
            redirect("../CustomerInventory.php");
            die();
        }

        if ($this->userModel->isEmailExist($data['usersEmail'])) {
            $_SESSION['flash-msg'] = 'email-exist';
            redirect("../CustomerInventory.php");
            die();
        }

        if ($this->userModel->isUsernameExist($data['usersName'])) {
            $_SESSION['flash-msg'] = 'username-exist';
            redirect("../CustomerInventory.php");
            die();
        }
